...
/** Download every imported execution as one flat CSV. */
export function downloadCsv() {
  const { csv, fills } = serializeFillsCsv();
  if (fills === 0) {
  ...
  a.href = url;
  a.download = `pnl-fills-${new Date().toISOString().slice(0, 10)}.csv`;
  a.click();
  URL.revokeObjectURL(url);
  toast.success(`Downloaded ${fills} fills`);
}
